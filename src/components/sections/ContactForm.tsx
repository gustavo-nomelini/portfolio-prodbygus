...
const ContactForm = () => {
  ...
  const requiredFieldMark = <span className="text-[var(--color1)] ml-1">*</span>;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <label htmlFor="phone" className="block text-sm font-medium text-[var(--foreground-muted)] mb-2">
          Telefone <span className="text-[var(--foreground-muted)] text-xs">(opcional)</span>
        </label>
        <input
          type="tel"
          id="phone"
        ...
        <div className={`p-4 rounded-md ${
          ...
          {submitStatus.type === 'success' && (
            <div className="w-full bg-gray-200 h-1 mt-2 rounded overflow-hidden">
              <div className="bg-green-500 h-1 animate-shrink-width" style={{animationDuration: '10s'}}></div>
            </div>
          )}
        </div>
      )}
